...
import { MapPin, Globe, ChevronDown, Check, Sparkles } from "lucide-react";
...
export const CountryCityPicker: React.FC<Props> = ({
  ...
  return (
    <div className={compact ? "space-y-1.5" : "space-y-2.5"}>
      {/* 1. Ligne des sélecteurs Pays et Ville */}
      <div className={compact ? "grid grid-cols-2 gap-2" : "grid grid-cols-1 sm:grid-cols-2 gap-3"}>
        ...
        <div>
          ...
          <div className="relative">
            <select
              ...
              className={`w-full pl-2 pr-6 ${compact ? "py-1.5 text-xs rounded-lg" : "py-2 text-xs sm:text-sm rounded-xl"} bg-white border border-slate-200 font-medium text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-600/20 focus:border-blue-600 transition-all appearance-none cursor-pointer`}
            >
        ...
        {/* CHAMP DE VILLE SÉLECTIONNÉE / SAISIE */}
        <div>
          <label className="block text-[10px] font-bold text-slate-700 mb-0.5 flex items-center justify-between">
            <span className="flex items-center gap-1">
            ...
            <span className="text-[9px] text-slate-400 font-medium">
              {cities.length > 0 ? `${cities.length} villes` : "Libre"}
            </span>
          </label>
      ...
      {/* 2. RUBAN DÉROULANT DE SÉLECTION RAPIDE DES VILLES SELON LE PAYS */}
      {cities.length > 0 && (
        <div className={compact ? "pt-0.5" : "pt-1"}>
          <div className="flex items-center justify-between mb-1">
            <span className={`${compact ? "text-[9.5px]" : "text-[10.5px]"} font-bold text-slate-600 flex items-center gap-1`}>
              <Sparkles className={`${compact ? "w-2.5 h-2.5" : "w-3 h-3"} text-blue-600`} />
              <span>Villes ({currentCountry}) en 1 clic :</span>
            </span>
            <span className={`${compact ? "text-[8.5px]" : "text-[9.5px]"} text-blue-600 font-semibold`}>
              Défiler →
            </span>
          </div>
            ...
            {cities.map((city) => {
              ...
              return (
                <button
                  ...
                  className={`shrink-0 ${compact ? "px-2 py-0.5 text-[10.5px] rounded-md" : "px-2.5 py-1 text-xs rounded-lg"} font-semibold transition-all flex items-center gap-1 cursor-pointer select-none ${
                    isSelected
                      ? "bg-blue-600 text-white shadow-xs ring-1 ring-blue-600 font-bold scale-[1.02]"
                      : "bg-white hover:bg-blue-50 text-slate-700 hover:text-blue-700 border border-slate-200/90"
                  }`}
                  title={`Choisir ${city}`}
                >
                  {isSelected && <Check className={`${compact ? "w-2.5 h-2.5" : "w-3 h-3"} text-white stroke-[3]`} />}
                  <span>{city}</span>
                </button>
              );
            })}
