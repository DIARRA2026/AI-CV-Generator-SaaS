      ...
      <div className="bg-white border-b border-slate-200 px-4 sm:px-6 py-2.5 flex flex-wrap items-center justify-between gap-3 no-print">
        ...
        <div className="flex items-center gap-3">
          <input
            ...
            title={dict.creator?.renameTooltip || "Cliquez pour renommer ce CV"}
          />
          <button
            type="button"
            onClick={handleResetData}
            title={dict.creator?.resetTooltip || "Réinitialiser les données"}
          ...
          >
            {cloudSyncStatus === "saving" ? (
              <span className="flex items-center gap-1 text-blue-700 bg-blue-50 border border-blue-200 px-2 py-0.5 rounded-md text-[10px] font-bold">
                ...
                <span>{dict.creator?.syncing || "Sync..."}</span>
              </span>
            ) : cloudSyncStatus === "saved" ? (
              <span className="flex items-center gap-1 text-emerald-700 bg-emerald-50 border border-emerald-200 px-2 py-0.5 rounded-md text-[10px] font-bold">
                ...
                <span>{dict.creator?.savedStatus || "Sauvegardé"} ✓</span>
              </span>
            ) : (
              <span className="flex items-center gap-1 text-slate-500 bg-slate-100 px-2 py-0.5 rounded-md text-[10px]">
                ...
                <span>{dict.creator?.localStatus || "Local"}</span>
              </span>
            )}
          </div>
        </div>
        ...
        <div className="hidden md:flex items-center gap-1.5 bg-slate-100/90 p-1 rounded-2xl border border-slate-200/80">
          ...
          >
            ...
            <span>{dict.creator?.atsAudit || "Score ATS"}</span>
          </button>
          ...
          >
            ...
            <span>{dict.creator?.coverLetterBtn || "Lettre IA"}</span>
          </button>
          ...
          >
            ...
            <span>{dict.creator?.jobAppBtn || "Demande d'Emploi"}</span>
          </button>
          ...
          >
            ...
            <span>{dict.creator?.portfolioBtn || "Portfolio Web"}</span>
          </Link>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            title={dict.common?.share || "Partager"}
          >
            <Share2 className="w-3.5 h-3.5 text-slate-500" />
            <span className="hidden sm:inline">{dict.common?.share || "Partager"}</span>
          </button>
          ...
          >
            <FileText className="w-3.5 h-3.5 text-indigo-600" />
            <span>{dict.creator?.exportDocx || "Word (.docx)"}</span>
          </button>
          ...
          >
            ...
            <span>
              {isDownloading
                ? dict.common?.loading || "Génération..."
                : downloadSuccess
                ? dict.common?.success || "Téléchargé !"
                : dict.creator?.exportPdf || "Télécharger PDF"}
            </span>
          </button>
        </div>
      </div>
        ...
        >
          ...
          <div className="bg-white p-2.5 sm:p-3 rounded-2xl border border-slate-200 mb-3 flex flex-wrap items-center justify-between gap-2 no-print shadow-xs">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-bold text-slate-800 flex items-center gap-1.5">
                ...
                {dict.creator?.previewA4Title || "Aperçu Page A4"}
              </span>
              {isEnterpriseActive ? (
                <span className="hidden sm:inline-flex items-center gap-1 text-[10px] font-bold text-amber-700 bg-amber-50 px-2.5 py-0.5 rounded-full border border-amber-200">
                  ...
                  {dict.creator?.unlockedBadge || "Offres Personnelles Offertes"}
                </span>
                ...
                <span className="hidden sm:inline-flex items-center gap-1 text-[10px] font-bold text-emerald-700 bg-emerald-50 px-2.5 py-0.5 rounded-full border border-emerald-200">
                  ...
                  {dict.creator?.unlockedBadge || "Profil Débloqué"}
                </span>
              ) : (
                <button
                  type="button"
                  ...
                  title={dict.creator?.removeWatermarkBtn || "Supprimer le filigrane pour ce profil"}
                >
                  <Sparkles className="w-3 h-3 text-blue-600" />
                  {dict.creator?.removeWatermarkBtn || "Retirer le filigrane (1 500 F)"}
                </button>
              )}
            </div>
            ...
            <div className="flex items-center gap-1">
              ...
              >
                ...
                <span className="hidden sm:inline">{dict.creator?.adjustBtn || "Ajuster"}</span>
              </button>
          ...
          {!isEnterpriseActive && licenseStatus.isNameChangedFromPrimary && (
            <div className="mb-3 p-3.5 bg-amber-50 border border-amber-200 rounded-2xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-xs shadow-xs no-print">
              <div className="flex items-start gap-2.5 text-amber-900">
                ...
                <div>
                  <span className="font-bold block text-slate-900">
                    {dict.creator?.newIdentityTitle || "Nouvelle identité détectée"}
                  </span>
                  <span className="text-slate-600 text-[11px] leading-relaxed">
                    {dict.creator?.newIdentityDesc || "Votre formule a activé un autre profil. Activez ce nouveau profil ou passez au Pack Multi-Profils."}
                  </span>
                </div>
              </div>
              <button
              ...
              >
                {dict.creator?.removeWatermarkBtn || "Activer ce profil (1 500 F)"}
              </button>
            </div>
          )}
      ...
      <div className="fixed bottom-5 right-5 z-30 xl:hidden no-print">
        {viewTab === "editor" ? (
          ...
          >
            ...
            <span>{dict.creator?.previewView || "Voir le CV (Aperçu A4)"}</span>
          </button>
        ) : (
          <button
            type="button"
          ...
          >
            ...
            <span>{dict.creator?.inputView || "Retourner à la Saisie"}</span>
          </button>
        )}
      </div>
