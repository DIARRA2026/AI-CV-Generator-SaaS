...
export const CountryCityPicker: React.FC<Props> = ({
  ...
  const handleSelectCountry = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    // Si la ville actuelle n'appartient pas au nouveau pays, pré-sélectionner la première ville phare
    ...
    <div className="space-y-3">
      ...
      <div>
        <label className="block text-xs font-bold text-slate-700 mb-1 flex items-center justify-between">
          ...
          <span className="text-[10px] text-slate-400 font-medium">Tous les pays du monde</span>
        </label>
        <div className="relative">
          <select
            value={currentCountry}
            onChange={handleSelectCountry}
            className="w-full pl-3 pr-8 py-2.5 bg-white border border-slate-200 rounded-xl text-xs sm:text-sm font-medium text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-600/20 focus:border-blue-600 transition-all appearance-none cursor-pointer"
          >
      ...
      {/* 2. CHAMP VILLE & RUBAN DYNAMIQUE DES VILLES DU PAYS */}
      <div>
        <label className="block text-xs font-bold text-slate-700 mb-1 flex items-center justify-between">
          <span className="flex items-center gap-1.5">
          ...
          <span className="text-[10px] text-slate-400 font-medium">
            {cities.length > 0 ? `${cities.length} villes suggérées` : "Saisie libre"}
          </span>
        </label>

        {/* Saisie texte manuelle ou via datalist */}
        <div className="relative">
          <input
            type="text"
            ...
            className="w-full pl-3.5 pr-3 py-2.5 bg-white border border-slate-200 rounded-xl text-xs sm:text-sm font-medium text-slate-800 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-600/20 focus:border-blue-600 transition-all"
          />
        ...
        {/* 3. RUBAN HORIZONTAL DE SÉLECTION RAPIDE DES VILLES */}
        {cities.length > 0 && (
          <div className="mt-2">
            <div className="flex items-center justify-between mb-1">
              <span className="text-[10.5px] font-bold text-slate-500 flex items-center gap-1">
                <span>Ruban des villes ({currentCountry}) :</span>
              </span>
              <span className="text-[9.5px] text-blue-600 font-medium">1 clic pour choisir</span>
            </div>
                ...
                return (
                  <button
                    ...
                    className={`shrink-0 px-2.5 py-1 rounded-lg text-xs font-semibold transition-all flex items-center gap-1 cursor-pointer ${
                      isSelected
                        ? "bg-blue-600 text-white shadow-xs scale-102 ring-1 ring-blue-600"
