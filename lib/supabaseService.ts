  ...
  static async syncSessionUser(authUser: any, token?: string): Promise<UserSession | null> {
    ...
    if (!cloudSub && resolvedPlan !== "free") {
      ...
      cloudSub = {
        ...
        currency: "FCFA",
        status: "pending",
        paymentMethod: "Accès Libre MonCV.ai",
      ...
      if (typeof window !== "undefined" && cleanEmail) {
        await fetch("/api/subscriptions", {
          ...
          body: JSON.stringify({
            ...
            status: subStatus,
            paymentMethod: details?.paymentMethod || "Accès Libre MonCV.ai",
    ...
    if (this.isAvailable() && supabase) {
      try {
        ...
        const subPayload: any = {
          ...
          currency: details?.currency || "FCFA",
          status: subStatus,
          payment_method: details?.paymentMethod || "Accès Libre MonCV.ai",
          phone_number: details?.phoneNumber || user?.phone,
          transaction_ref: details?.transactionRef || `TRX_${Date.now()}`,
