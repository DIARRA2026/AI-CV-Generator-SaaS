...
export const TemplateElegant: React.FC<TemplateProps> = ({ data }) => {
  ...
  return (
    <div className="p-9 min-h-[297mm] bg-white text-slate-800 font-serif text-[11px] leading-relaxed flex flex-col justify-between">
      <div>
        ...
        {summary && (
          <div className="mb-5">
            ...
            <p className="text-slate-700 italic text-[10.5px] leading-relaxed text-justify">
              {summary}
            </p>
          </div>
        )}
        ...
        <div className="grid grid-cols-3 gap-6">
          ...
          <div className="col-span-2 space-y-5">
            {experiences && experiences.length > 0 && (
              <div>
                <h2
                  className="text-[10px] uppercase tracking-widest font-bold font-sans border-b pb-1 mb-3"
                  style={{ color, borderColor: `${color}30` }}
                >
                ...
                <div className="space-y-4">
                  {experiences.map((exp) => (
                    <div key={exp.id} className="pl-3 border-l-2" style={{ borderColor: `${color}40` }}>
                      ...
                      <p className="text-[10px] font-sans font-medium text-slate-600 mb-1.5">
                        {exp.company}{exp.city ? ` | ${exp.city}` : ""}
                      </p>
                      <ul className="list-disc list-outside ml-4 space-y-0.5 text-slate-700 text-[10px] font-sans">
          ...
          <div className="col-span-1 space-y-5">
                    ...
                    <div key={cat.id}>
                      ...
                      <div className="flex flex-wrap gap-1">
                        {cat.items.map((item, idx) => (
                          <span
                            key={idx}
                            className="inline-block px-2 py-1 bg-slate-100 text-slate-700 rounded text-[9px] font-medium leading-tight border border-slate-200"
                          >
                            {item}
                          </span>
                        ))}
            ...
            {languages && languages.length > 0 && (
              <div>
                <h2
                  className="text-[10px] uppercase tracking-widest font-bold font-sans border-b pb-1 mb-2.5"
                  style={{ color, borderColor: `${color}30` }}
                >
                  Langues
                </h2>
